...
const MyPetAdoptionListings = () => {
  const { user, isLoading } = useAuth0();
  const [userEmail, setUserEmail] = useState<string | null>(null);
  const [myAdoptPets, setMyAdoptPets] = useState<lostPetProps[]>([]);
  ...
  useEffect(() => {
    const fetchMyLostPets = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.post(`${BASE_URL}/api/v1/my/pets/adopt`, {
          email: userEmail,
        });
        setMyAdoptPets(response.data.pets || []);
      } catch (error) {
  ...
  const handleUpdatePet = async () => {
    if (!userEmail) return;
    try {
      const response = await axios.put(`${BASE_URL}/api/v1/pet/adopt/update`, {
        id: petId,
        owner: userEmail,
        name: petName,
        description,
  ...
  const handleDeletePet = async (petId: string) => {
    ...
    try {
      const response = await axios.delete(`${BASE_URL}/api/v1/pet/delete`, {
        data: { id: petId, owner: userEmail },
      });
      if (response.data.success) {
        setMyAdoptPets((prev) => prev.filter((pet) => pet._id !== petId));
    ...
    <>
      {loading || isLoading ? (
        ...
        <Table className="w-full mt-4">
          ...
          <TableBody>
            {myAdoptPets.length > 0 ? (
              myAdoptPets.map((pet: adoptPetProps) => (
...
export default MyPetAdoptionListings;
