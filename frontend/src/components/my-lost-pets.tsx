...
const MyLostPets = () => {
  ...
  const [myLostPets, setMyLostPets] = useState<lostPetProps[]>([]);
  ...
  // Update userEmail when Auth0 user is available
  useEffect(() => {
    if (user?.email) {
  ...
  useEffect(() => {
    const fetchMyLostPets = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.post(`${BASE_URL}/api/v1/my/pets/lost`, {
          email: userEmail,
        });
        setMyLostPets(response.data.pets || []);
      } catch (error) {
  ...
  const handleUpdatePet = async () => {
    if (!userEmail) return;
    try {
      const response = await axios.put(`${BASE_URL}/api/v1/pet/lost/update`, {
        id: petId,
        owner: userEmail,
        name: petName,
        description,
  ...
  const handleDeletePet = async (petId: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        setMyLostPets((prev) => prev.filter((pet) => pet._id !== petId));
